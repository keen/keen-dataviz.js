...
export const Dataviz = function (options = {}) {
  ...
  const defaultOptions = {
    ...
    data: {
     ...
     onclick: (d, element) => copyToClipBoard(d.value),
   },
   grid: {
     y: {
       show: true
  ...
  if (!this.config.utils.clickToCopyToClipboard) {
    this.config.data.onclick = () => {};
  }

  if (options.data && options.data.onclick) {
    this.config.data.onclick = (d, element) => {
      const callback = options.data.onclick.bind(null, d, element);
      callback();

      if (this.config.utils.clickToCopyToClipboard) copyToClipBoard(d.value);
    };
  }
