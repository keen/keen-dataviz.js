...
function generateAreaWithMetricsDom(prev, curr) {
  const prevResult = prev.result || 0;
  const currResult = curr.result || 0;
  const count = currResult - prevResult;
  let labelClass;
  let iconClass = 'arrow-green';
  let percentDifference = Math.round((currResult / prevResult) * 100);
...
function buildDomWrapper(el, options) {
  ...
  html += `<div class="${options.theme}${options.showMetrics ? ' relative' : ''}">`;
  if (options.title && options.showTitle) {
    html += `<div class="keen-dataviz-title ${options.theme}-title">${options.title}</div>`;
  }
  if (options.showMetrics && options.type === 'area-spline') {
    if (!options.currentQuery || !options.previousQuery) {
      html += '<div class="no-data">No data to display</div>';
    } else {
      html += generateAreaWithMetricsDom(options.previousQuery, options.currentQuery);
    }
  }
