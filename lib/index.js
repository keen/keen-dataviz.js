...
export const Dataviz = function (options = {}) {
  ...
  if(this.config.type.includes('-range')) {
    ...
    this.config.colors = ['#ff0000', '#ff0000'];
  }
...
Dataviz.prototype.render = function(results = undefined){
  ...
  // range charts cheat for Min stacked values visibility
  if(datavizInstance.config.range){
    if(datavizInstance.config.type.includes('bar')){
      return document.querySelector('.c3-target-Min').style.display = 'none';
    }
    return document.querySelector('.c3-area-Min').style.display = 'none';
  }
