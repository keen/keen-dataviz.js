...
export const Dataviz = function (options = {}) {
  ...
  const defaultOptions = {
    container: undefined, // querySelector, for example '#someDiv', or DOM node
...
function buildDomWrapper(el, options) {
  ...
  if (options.legend.position === 'top' || options.legend.position === 'left') {
    container = `<div class="keen-c3-legend keen-c3-legend-${options.legend.position}"></div>${container}`;
  } else {
    container = `${container}<div class="keen-c3-legend keen-c3-legend-${options.legend.position}"></div>`;
  }
  html += `<div class="${options.theme}">`;
  if (options.title) {
    html += `<div class="${options.theme}-title">${options.title}</div>`;
  }
  html += `<div class="${options.theme}-stage"><div class="${options.theme}-rendering">${container}</div></div>`;
  if (options.notes) {
