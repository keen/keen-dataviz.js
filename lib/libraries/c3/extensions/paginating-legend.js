...
export default function (options) {
  ...
  const columns = [];
  const domNode = this.el().querySelector('.' + this.theme() + '-rendering');
  ...
  function renderLegendComponent(){
    ...
    legendItemList.enter()
        ...
        const elementText = d3.select(this)
          ...
          elementText.text(function(id){
            if (d3.select(this).node().getBBox().width > 105) {
              return id.length <= options.legend.label.textMaxLength ? id : id.substring(0, options.legend.label.textMaxLength) + '...';
            }
            else {
              return id;
            }
      ...
      .on('mouseout', function (id) {
        ...
        d3.selectAll('.' + self.config.theme + '-rendering .keen-c3-legend-label-overlay')
          .remove();
      })
