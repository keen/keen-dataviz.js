...
function defineC3(){
  ...
  function getDefaultOptions(){
    ...
    const ENFORCED_OPTIONS = {
      bindto: this.el().querySelector('.' + this.config.theme + '-rendering'),
      color: {
      ...
      size: {
        height: this.config.height ? this.config.height - this.el().offsetHeight : 400,
      ...
      render: function(){
        ...
        if (options.legend.show === true
          // && options.legend.position === 'right'
            // w? && ['gauge'].indexOf(type.replace('horizontal-', '')
          ) {
                ...
                options.legend.show = false;
                const paddedWidth = this.el().querySelector('.' + this.config.theme + '-rendering').offsetWidth - 100;
                options.size.width = options.size.width || paddedWidth;
                this.el().querySelector('.' + this.theme() + '-rendering').style['margin-' + options.legend.position]=this.config.legend.width;
