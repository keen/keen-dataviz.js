  ...
  each(c3Types, function(type, index) {
    types[type] = {
      render: function(){
        ...
        // 100% for bar charts
        const sumArray = calculateSumsForBarsPercents(this.dataset.matrix);
        if ((options.stacking === 'percent' && type === 'bar') || (options.stacking === 'percent' && type === 'horizontal-bar')) {
          this.dataset.matrix = [this.dataset.matrix[0], ...calculatePercentsForBars(this.dataset.matrix, sumArray)];
          options.axis = {
            y: {
              padding: {
                top: 0,
            ...
            format: {
              ...
              value: (value, ratio, id, index) => {
                ...
                if (this.config.tooltip && this.config.tooltip.format && this.config.tooltip.format.value)
                {
                  ...
                  // Restore value from percents calculation for stacking percents bar charts
                  if ((options.stacking === 'percent' && type === 'bar') || (options.stacking === 'percent' && type === 'horizontal-bar')) {
...
function calculateSumsForBarsPercents(matrix) {
  const sumArray = [];
...
function calculatePercentsForBars(matrix, sumArray) {
