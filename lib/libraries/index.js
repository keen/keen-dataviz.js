      ...
      render: function(){
        ...
        if (type === 'gauge') {
          ...
          if (options.visibilityThreshold) {
            var otherTotal = 0;
            var total = this.data().slice(1)
              .map(function(item) { return item[1] })
              .reduce(function(prev, curr) { return prev + curr; }, 0);

            this.data().slice(1).forEach(function(item) {
              if ((item[1] / total) > options.visibilityThreshold) {
                options.data.columns.push(item);
              }
              else {
                otherTotal += item[1];
              }
            ...
            if (otherTotal > 0) {
              options.data.columns.push(['Other', otherTotal]);
            }
          }
          else {
